...
def do_update_translations(app=("a", ""), lang=("l", ""),
  ...
  if lang:
    filepath = path.join(root, lang, 'LC_MESSAGES', 'messages.po')
  ...
  for lang_dir in listdir(root):
    filename = path.join(root, lang_dir, 'LC_MESSAGES', 'messages.po')
    if lang and filename != \
                       path.join(root, lang, 'LC_MESSAGES', 'messages.po'):
      continue
    if path.exists(filename):
      ...
      f = file(filename)
      try:
        catalog = read_po(f, locale=locale, domain='messages')
      finally:
        f.close()
